...
def main():
    ...
    with requests.Session() as s:
        ...
        while tick > 2 and tick < 298:
            ...
            orders = get_orders(s, 'OPEN')
            algo_close = ticker_close(s, 'ALG')
            bid, ask = ticker_bid_ask(s, 'ALG')

            # This is getting information about or position sizing and assiging variables based off of the size
            position = s.get('http://localhost:9999/v1/securities?ticker=ALG').json()
            myposition = position[0]['position']
            if myposition >= 15000:
                BUY_SCALP_VOLUME = 0
                SELL_SCALP_VOLUME = 5000
            ...
            #  This is placeing orders using the above criteriea that was assigned based off of our position sizing.
            if ask - bid > .03:
                buy_bid(s, 'ALG', bid, BUY_SCALP_VOLUME)
                sell_ask(s, 'ALG', ask, SELL_SCALP_VOLUME)
                buy_bid(s, 'ALG', bid, BUY_SCALP_VOLUME)
                sell_ask(s, 'ALG', ask, SELL_SCALP_VOLUME)
                buy_bid(s, 'ALG', bid, BUY_SCALP_VOLUME)
                sell_ask(s, 'ALG', ask, SELL_SCALP_VOLUME)
            ...
            orders = get_orders(s, 'OPEN')
            algo_close = ticker_close(s, 'ALGOO')
            bid, ask = ticker_bid_ask(s, 'ALGOO')

            # This is getting information about or position sizing and assiging variables based off of the size
            position = s.get('http://localhost:9999/v1/securities?ticker=ALGOO').json()
            myposition = position[0]['position']
            if myposition >= 15000:
                BUY_SCALP_VOLUME = 0
                SELL_SCALP_VOLUME = 5000
            ...
            #  This is placeing orders using the above criteriea that was assigned based off of our position sizing.
            if ask - bid > .03:
                buy_bid(s, 'ALGOO', bid, BUY_SCALP_VOLUME)
                sell_ask(s, 'ALGOO', ask, SELL_SCALP_VOLUME)
                buy_bid(s, 'ALGOO', bid, BUY_SCALP_VOLUME)
                sell_ask(s, 'ALGOO', ask, SELL_SCALP_VOLUME)
                buy_bid(s, 'ALGOO', bid, BUY_SCALP_VOLUME)
                sell_ask(s, 'ALGOO', ask, SELL_SCALP_VOLUME)
